...
public class Player extends AppCompatActivity {
    ...
    static MediaPlayer mMediaPlayer , play;
    int position;
    TextView curTime;
    TextView totTime;
    ImageView playIcon;
...
//            Log.i("handler ", "handler called");
    ...
    private ArrayList<File> random(){

        //final ArrayList<File> allSongs = findMusics(Environment.getExternalStorageDirectory());
        ArrayList<File> allSongs = (ArrayList) bundle.getParcelableArrayList("songs");
    ...
    private void check(){
       /* if (repeatIcon.getDrawable().getConstantState() == getResources().getDrawable( R.drawable.repeat_default).getConstantState())
        {
            repeatIcon.setImageResource(R.drawable.repeat_one);
            int time = mMediaPlayer.getCurrentPosition();
            for(int f= time ; f == mMediaPlayer.getDuration();){
                mMediaPlayer.seekTo(0);
                mMediaPlayer.start();
            }
        }*/
       /* else */if(repeatIcon.getDrawable().getConstantState() == getResources().getDrawable( R.drawable.repeat_default).getConstantState())
        {
            repeatIcon.setImageResource(R.drawable.repeat_all);
            mMediaPlayer.setLooping(true);
        }
        else if(repeatIcon.getDrawable().getConstantState() == getResources().getDrawable(R.drawable.repeat_all).getConstantState())
        {
